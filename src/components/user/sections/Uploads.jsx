...
import { FaRegFile, FaCheckCircle, FaVideo } from "react-icons/fa";
...
const VIDEO_ALLOWED_EXTS = [".mp4", ".mov", ".avi", ".webm", ".m4v", ".mpeg", ".mpg"];
...
const VIDEO_MAX_FILE_SIZE = 15 * 1024 * 1024; // 15 MB per file
...
const Uploads = ({
  ...
  const handleFileChange = (e) => {
    ...
    // Prevent duplicates: check by name + size against already selected files
    const existingKeys = new Set(
      files.map((f) => `${f.file.name}_${f.file.size}`)
    );

    const duplicates = selected.filter(
      (file) => existingKeys.has(`${file.name}_${file.size}`)
    );
    ...
    if (duplicates.length > 0) {
      setUploadError(
        duplicates.length === 1
          ? `${duplicates[0].name} is already selected.`
    ...
    const newFiles = uniqueSelected.map((file) => ({
      file,
  ...
  useEffect(() => {
    return () => {
      files.forEach((f) => f.preview && URL.revokeObjectURL(f.preview));
      videoFiles.forEach((f) => f.preview && URL.revokeObjectURL(f.preview));
    };
  ...
  const validateVideoFile = (file) => {
    const ext = "." + (file.name.split(".").pop() || "").toLowerCase();
    if (!VIDEO_ALLOWED_TYPES.includes(file.type) && !VIDEO_ALLOWED_EXTS.includes(ext)) {
      return "Only MP4, MOV, AVI, WEBM, M4V, MPEG, MPG allowed";
    }
    if (file.size > VIDEO_MAX_FILE_SIZE) {
      return `File exceeds 15 MB (${formatSize(file.size)})`;
    }
    return "";
  };
  ...
  const handleVideoFileChange = (e) => {
    ...
    // Prevent duplicates
    const existingKeys = new Set(
      videoFiles.map((f) => `${f.file.name}_${f.file.size}`)
    );
    ...
    const newVideoFiles = uniqueSelected.map((file) => ({
      file,
      preview: file.type.startsWith("video/") ? URL.createObjectURL(file) : null,
      error: validateVideoFile(file),
    }));
  ...
  const handleVideoUpload = async () => {
    const valid = videoFiles.filter((f) => !f.error);
    if (valid.length === 0) {
      setVideoUploadError("Please select valid video files to upload.");
      return;
    }
    ...
    try {
      const formData = new FormData();
      valid.forEach((f) => formData.append("files", f.file));
      formData.append("description", "Product reel");
      formData.append("tags", "video,instagram,reel");
      formData.append("is_favorite", "false");

      await apiFetch("api/assets/upload-video", {
        method: "POST",
        body: formData,
      });
      ...
      setVideoUploadError(error?.data?.message || error?.message || "Upload failed. Please try again.");
    } finally {
      setVideoUploading(false);
    }
  };

  const videoValidCount = videoFiles.filter((f) => !f.error).length;
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-5 mt-5">
        ...
        >
          ...
          <span className="text-xs text-[#94a3b8] mt-1 text-center px-2">
            MP4, MOV, AVI, WEBM · max 15 MB each
          </span>
        </div>
        ...
        <input
          ref={videoInputRef}
          type="file"
          accept=".mp4,.mov,.avi,.webm,.m4v,.mpeg,.mpg,video/mp4,video/quicktime,video/x-msvideo,video/webm,video/x-m4v,video/mpeg,video/mpg"
          multiple
      ...
      {/* Selected files sections — slide down with transition */}
      <div
        className="overflow-hidden transition-all duration-500 ease-in-out"
        style={{
          maxHeight: hasFiles || hasVideoFiles ? "500px" : "0px",
          opacity: hasFiles || hasVideoFiles ? 1 : 0,
          marginTop: hasFiles || hasVideoFiles ? 20 : 0,
        }}
      >
        {/* Image files list */}
        {hasFiles && (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className={`text-sm font-medium ${isdark ? "text-white" : "text-[#374151]"}`}>
                {files.length} image{files.length !== 1 ? "s" : ""} selected · {validCount} valid
              </span>
              ...
              {files.map((item, i) => (
                ...
                >
                  ...
                  <div className="px-1.5 py-1">
                    <p className={`text-xs truncate ${isdark ? "text-white" : "text-[#374151]"}`}>
                      {item.file.name}
                    </p>
        ...
        {hasVideoFiles && (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className={`text-sm font-medium ${isdark ? "text-white" : "text-[#374151]"}`}>
                {videoFiles.length} video{videoFiles.length !== 1 ? "s" : ""} selected · {videoValidCount} valid
              </span>
              <div className="flex gap-2">
                <button
                ...
                <button
                  ...
                  disabled={videoUploading || videoValidCount === 0}
                  className="flex items-center gap-2 text-xs px-3 py-1 rounded bg-[#8b5cf6] text-white cursor-pointer disabled:opacity-50"
                >
                  {videoUploading ? (
                    <>
                    ...
                    <>
                      <FaCheckCircle size={13} />
                      Upload {videoValidCount > 0 ? `(${videoValidCount})` : ""}
                    </>
                  )}
                </button>
              </div>
                ...
                >
                  <div className="h-20 w-full bg-slate-100 flex items-center justify-center overflow-hidden">
                    {item.preview ? (
                      ...
                      <FaVideo className="text-slate-400" size={20} />
                    )}
                  </div>
                  <div className="px-1.5 py-1">
                    <p className={`text-xs truncate ${isdark ? "text-white" : "text-[#374151]"}`}>
                  ...
                  <button
                    onClick={() => handleRemoveVideoFile(i)}
                    className="absolute top-1 right-1 h-5 w-5 flex items-center justify-center rounded-full bg-black/60 text-white"
                  >
                    <IoMdClose size={11} />
                  </button>
            ...
            {videoUploadError && (
              <p className="text-xs text-red-400 mt-3">{videoUploadError}</p>
            )}
          </div>
        )}
      </div>
