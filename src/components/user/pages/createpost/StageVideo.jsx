...
// Fallback video placeholder (a sample MP4 from W3C)
...
// Each item: { day, scheduled_at, content, day_group_id, post_ids, video_url }
...
const StageVideo = ({
  ...
  // regenerated video in the review/preview.
  useEffect(() => {
  ...
  // Regenerate the video for the current day only.
  const handleUpdate = async () => {
    ...
    try {
      ...
      messageApi.success(
        result?.message || d?.message || "Video regenerated successfully"
      );
    } catch (error) {
      ...
      const errMsg =
        errResult?.message || error?.message || "Failed to regenerate video";
  ...
  const handleApprove = () => {
    ...
    modal.confirm({
      ...
      content:
        "Review each day's video, then Approve to lock it in and move to the next day — once approved a day can't be revisited.",
      okText: "Approve",
      cancelText: "Cancel",
    ...
    <ConfigProvider theme={antdTheme}>
      ...
      >
        ...
        <p className={`text-center text-sm mb-6 ${isdark ? "text-[#64748b]" : "text-[#94a3b8]"}`}>
          Review each day&apos;s video, then Approve to lock it in and move to the next day —
        ...
        {isComplete ? (
          ...
          <div className="max-w-md mx-auto mb-8">
            ...
            {/* Social-post style preview — video */}
            <div
                ...
                {current.video ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className={`text-xs ${isdark ? "text-[#475569]" : "text-gray-400"}`}>
                      {regenerating ? "Generating video…" : "No video yet — click Regenerate to generate"}
                    </p>
                  </div>
                )}
                {regenerating && (
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. cinematic pan around the product"
